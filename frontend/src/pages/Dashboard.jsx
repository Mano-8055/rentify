...
import {
  getOwnerRequests, acceptRental, rejectRental,
  completeRental, getMyRentals, markReturned,
...
const StatusBadge = memo(function StatusBadge({ status }) {
  return (
    <span className={`text-xs px-2.5 py-0.5 rounded-full border font-medium status-${status}`}>
...
/* ── Memoised stat card ── */
const StatCard = memo(function StatCard({ icon, value, label, card, val }) {
  return (
    <div className={`rounded-2xl p-5 border backdrop-blur-xl transition-all hover:scale-[1.02] ${card}`}>
...
export default function Dashboard() {
  ...
  const STATS = [
    { label: "My Listings",      value: myProducts.length,                  icon: "📦", card: "bg-green-500/5  border-green-500/20",  val: "text-green-400" },
    { label: "Pending Requests", value: pendingRequests.length,             icon: "⏳", card: "bg-yellow-500/5 border-yellow-500/20", val: "text-yellow-400" },
    { label: "Active Rentals",   value: activeRentals.length,               icon: "🔄", card: "bg-blue-500/5   border-blue-500/20",   val: "text-blue-400" },
    { label: "Trust Score",      value: `${user?.likelihoodPoints ?? 0}/100`, icon: "🛡️", card: "bg-purple-500/5 border-purple-500/20", val: "text-purple-400" },
  ];
          ...
          {pendingRequests.length === 0 ? (
            ...
            <div className="flex flex-col gap-3">
              {pendingRequests.map((r) => (
                <div key={r._id} className="glass rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:border-white/10 transition-colors">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500">
                      ...
                      {" · "}Trust:{" "}
                      <span className="text-green-400 font-medium">{r.renter?.likelihoodPoints}/100</span>
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button
                      ...
                      className="px-4 py-2 rounded-lg bg-green-500/15 text-green-400 border border-green-500/20 hover:bg-green-500/25 text-xs font-medium transition-all disabled:opacity-50 flex items-center gap-1.5"
                    >
                    ...
                    <button
                      ...
                      disabled={actionLoading === r._id}
                      className="px-4 py-2 rounded-lg bg-red-500/8 text-red-400 border border-red-500/20 hover:bg-red-500/18 text-xs font-medium transition-all disabled:opacity-50"
                    >
                      Reject
                    </button>
                  </div>
                ...
                <div key={r._id} className="glass rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <Link
                        ...
                        className="font-medium text-white text-sm hover:text-green-400 transition-colors"
                      >
                        {r.product?.name}
                      </Link>
                      <StatusBadge status={r.status} />
                  ...
                  {r.status === "accepted" && (
                    <button
                      ...
                      disabled={actionLoading === r._id}
                      className="px-4 py-2 rounded-lg text-xs font-medium btn-primary shrink-0 disabled:opacity-50 flex items-center gap-1.5"
                    >
        ...
        <section>
          ...
          {myProducts.length === 0 ? (
            <div className="glass rounded-2xl p-10 text-center">
              ...
              <p className="text-slate-500 text-sm mb-5">No products listed yet</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {myProducts.map((p) => (
                <div key={p._id} className="glass rounded-2xl p-4 hover:border-white/10 transition-all group">
                  ...
                  <button
                    ...
                    className="w-full text-xs py-2 rounded-lg bg-red-500/8 border border-red-500/18 text-red-400 hover:bg-red-500/18 transition-all"
                  >
                    Delete Listing
                  </button>
                </div>
